...
/// Caps keep `sum(qty_i * price_i)` for up to 8 lines far below `u64`
/// overflow, so the only observable failures are genuine logic bugs.
        ...
        let order_lines: Vec<OrderLine> = lines
            ...
            .map(|(index, &(_seed, quantity_value, price))| {
                OrderLine::new(
                    ...
                    Money::from_minor(price, currency.clone()),
                )
            })
            .collect();
        let expected: u64 = order_lines
            .iter()
            .map(|line| {
                line.unit_price().minor_units() * u64::from(line.quantity().value())
            })
            .sum();

        let order = Order::new(order_lines).unwrap();
